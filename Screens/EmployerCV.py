...
class EmployerCV(Container):
    def __init__(self,page:Page):
        ...
        jobtitle_input = Dropdown(width=250,autofocus=False,label="Job Title",label_style=TextStyle(color=Colors.BLUE,weight=FontWeight.W_600),border_radius=BorderRadius(10,10,10,10),border_width=1,max_menu_height=30,color ="#36618e",text_style=TextStyle(weight=FontWeight.W_500),options=Jobtitle_options)
        
        Worktype_options = [dropdown.Option(text=job,key=job) for job in self.emplower.Get_WorkTypes()]
        Worktype_input = Dropdown(width=250,autofocus=False,label="Work Type",label_style=TextStyle(color=Colors.BLUE,weight=FontWeight.W_600),border_radius=BorderRadius(10,10,10,10),border_width=1,max_menu_height=30,color ="#36618e",text_style=TextStyle(weight=FontWeight.W_500),options=Worktype_options)
        
        Gender_options = [dropdown.Option(text=job,key=job) for job in ["Male","Female"]]
        Gender_input = Dropdown(width=250,autofocus=False,label="Gender",label_style=TextStyle(color=Colors.BLUE,weight=FontWeight.W_600),border_radius=BorderRadius(10,10,10,10),border_width=1,max_menu_height=30,color ="#36618e",text_style=TextStyle(weight=FontWeight.W_500),options=Gender_options)
        
        self.description_Column = Column(controls=[self.description_header_row,Row(controls=[jobtitle_input,Worktype_input,Gender_input],alignment=MainAxisAlignment.CENTER)],expand=True,spacing=20,horizontal_alignment=CrossAxisAlignment.CENTER)
        ...
        uploadContainer = Container(content=Column(expand=True,controls=[
        ...
        self.upload_column = Column(controls=[self.upload_header,uploadContainer],expand=True,horizontal_alignment=CrossAxisAlignment.CENTER)
